...
final class MatrixD[W <: Nat, H <: Nat, A] private[iliad] (_unsized: Vector[A],
  ...
  //TODO: Should be called prettyPrint()
...
object MatrixAlgebra {
  implicit def matrix4Algebra[A: ConvertableFrom: ConvertableTo]: MatrixAlgebra[nat._4, A] =
...
private trait Matrix4Algebra[A] extends MatrixAlgebra[nat._4, A] {
  ...
  def times(m0: MatrixD[nat._4, nat._4, A],
            m1: MatrixD[nat._4, nat._4, A]): MatrixD[nat._4, nat._4, A] =
    new MatrixD(
        MatrixLibrary.multiplyMM(m0.map(F.toFloat).toArray, m1.map(F.toFloat).toArray)
          .toVector
          .map(T.fromFloat),
        m0.width,
        m0.height)
  ...
  def inverse(m: MatrixD[nat._4, nat._4, A]): MatrixD[nat._4, nat._4, A] =
    new MatrixD(MatrixLibrary.invertM(m.map(F.toFloat).toArray).toVector.map(T.fromFloat),
                m.width,
                m.height)
  ...
  def timesv(m0: MatrixD[nat._4, nat._4, A],
    ...
    MatrixD.sized(
        nat._1,
        nat._4,
        MatrixLibrary.multiplyMV(m0.map(F.toFloat).toArray, m1.map(F.toFloat).toArray)
          .toVector
          .map(T.fromFloat))
}
